...
function SaudePage() {
  const total = kpiResumo.saude;
  const unidades = unidadesSaude.length;
  const porUnidade = total / unidades;
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <ChartCard title="Distribuição dos recursos" subtitle="Por unidade de saúde">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={unidadesSaude} dataKey="valor" nameKey="unidade" outerRadius={100} label={(e) => e.unidade}>
                {unidadesSaude.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Pie>
          ...
          <tbody className="divide-y divide-border">
            {unidadesSaude.map((u) => (
              <tr key={u.unidade} className="hover:bg-accent/40">
                ...
                <td className="px-4 py-3 text-right tabular-nums">{formatBRL(Math.round(u.valor / u.atendimentos))}</td>
              </tr>
            ))}
          </tbody>
        </table>
