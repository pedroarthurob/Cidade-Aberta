...
function LicitacoesPage() {
  ...
  const empresas = useMemo(() => Array.from(new Set(licitacoes.map(l => l.empresa))), []);
  const anos = useMemo(() => Array.from(new Set(licitacoes.map(l => l.data.slice(0, 4)))), []);
  ...
  const filtered = licitacoes.filter((l) => {
    ...
    const matchA = ano === "Todos" || l.data.startsWith(ano);
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((l) => (
                  <tr key={l.id} className="transition hover:bg-accent/40">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">{new Date(l.data).toLocaleDateString("pt-BR")}</td>
