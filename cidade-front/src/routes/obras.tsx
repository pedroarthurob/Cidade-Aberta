...
function ObrasPage() {
  const [obrasList, setObrasList] = useState(obras);
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/obras")
      ...
      .catch(err => console.log("Failed to fetch obras:", err));
  }, []);

  const bairros = useMemo(() => Array.from(new Set(obrasList.map(o => o.bairro))), [obrasList]);
  const empresas = useMemo(() => Array.from(new Set(obrasList.map(o => o.empresa))), [obrasList]);

  const filtered = obrasList.filter((o) =>
